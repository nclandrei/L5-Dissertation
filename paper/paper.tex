...
\begin{itemize}
  ...
  \item more grammar errors imply increase \emph{Time-To-Close} values;
  ...
  \item verbose summary and description imply increased \emph{Time-To-Close} values.
\end{itemize}
...
tickets with archive we can see the longest \emph{Time-To-Close} mean while tickets having text attachments are at 
...
reduces the \emph{Time-To-Close} for the ticket. This can imply that developers find value in various steps to reproduce, 
config values used by users when the bug encountered or data in a spreadsheet format (e.g. csv) that can be 
found in such files and helps them lower the \emph{Time-To-Close} for the ticket.
...
that uploading a screenshot or a code snippet can help reduce the \emph{Time-To-Close} for the ticket.
...
As it can be shown in the figure, having steps to reproduce can significantly reduce the \emph{Time-To-Close} 
...
has a positive influence on the \emph{Time-To-Close}. One solution that we propose for maximizing their benefit 
...
This implies a statistically significant finding - as the number of comments increases, the \emph{Time-To-Close} value of the ticket also increases. 
