...
with multiple commands (i.e. store, analyse, plot, statistics) that can automatically fetch any number of tickets 
from a Jira instance, analyse them, generate plots and run statistical tests. 
...
Another research paper that strengthens the argument that readability is a quality 
...
the authors investigated whether tickets could be summarised automatically and efficiently. What 
...
the authors applied a card sort in order to organise the comments into hierarchies to 
identify common patterns. Their findings can be summarised in the following top seven
...
GNOME and Eclipse and projects and split their approach into four steps: extract and organise tickets, 
...
In this section, several state-of-the art research studies revolving around software tickets and their surrounding 
environment were presented. The findings and techniques summarised proved useful for our own research, 
helping us build the data set as we will elaborate in the following section.
...
fetching the tickets from any Jira instance, storing them into some form of database, analyse the variables of interest, 
...
This is the first command implemented in our tool - it is designed to fetch, from any valid Jira URL, any number of tickets 
and then store everything inside an instance of BoltDB \cite{bolt}, a key-value pair 
database. The whole process is parallelised with the possibility of scaling even up to hundreds of goroutines
...
Once the Bolt DB instance is completely populated with all the tickets we were interested in, the analyse command first 
...
outlined in Section \ref{intro}, each in its own separate goroutine in order to speed up the process:
\begin{itemize}
...
we passed a filter for not fetching tickets still marked as Open, there was a small number erroneously retrieved by Jira). 
...
For every type of analysis, we also discuss possible reasons why certain correlations occurred. Furthermore, for all 
...
we collected (22) are also written in Java, thus we had a considerable chunk of data to analyse.
...
1 to 1,600 total comment words per ticket (76,633 tickets) and for this majority of tickets, the trend is ascending.
We could not generate the plot based solely on these values as there would have been 36,711 tickets left out. 
Therefore, the curve is skewed to the right, although the bulk of tickets that dictate the trend, are on the left side.

A correlational analysis run on all 201,786 tickets, however, revealed a weak positive relationship with $r = 0.271$ and $p < 0.01$. 
...
in summary and description increases, the Time-To-Close will decrease. However, when numbers are run, the opposite trend 
surfaces. The majority of the tickets are in the range of 2 to 290 words, but as in the case of comments complexity analysis, 
we could not generate the plot only for those as we would have left 61,322 tickets out (out of a total of 201,786 tickets analysed). 
Therefore, the curve is skewed to the right, although the bulk of tickets that dictate the trend, are on the left side.
...
Zimmermann et al. \cite{zimmermann2010makes} present in their study is that a good, succinct summary can significantly 
...
with more than 4,000 words. Even though they included various types of information, including steps to reproduce, 
...
and description where administrators can write succinct messages advising reporters to not write verbose paragraphs 
...
in the code of conduct for the project stating that everything should be 100\% grammatically correct, many reporters with a 
...
threshold and it is believed that they are becoming rude or impolite, there could be a warning/pop-up on the screen advising 
...
Crowston et al. \cite{crowston2012free} present in their study, open source projects lack organised coordination which,
